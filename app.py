...
def get_text_from_image(image):
    # AI ko prompt de rahe hain ki photo ko kaise padhna hai aur format karna hai
    prompt = """
    ...
    # Remove markdown code blocks if AI accidentally adds them
    clean_html = response.text.replace("```html", "").replace("```", "")
    return clean_html

def create_pdf(all_html_content):
    # Ye HTML design bilkul aapke screenshot jaisa 2-Column layout banayega
    full_html = f"""
    <!DOCTYPE html>
    <html>
    <head>
        ...
        <style>
            ...
            .question {{
                margin-bottom: 20px;
                page-break-inside: avoid; /* Question ko aadha katne se rokega */
            }}
            p {{
                margin: 2px 0;
            }}
    ...
    # PDF generation settings (A4 size)
    options = {
        'page-size': 'A4',
        'margin-top': '0.75in',
        'margin-right': '0.75in',
    ...
    # HTML ko PDF mein convert karna
...
# Multiple photos upload karne ka option
...
if uploaded_files:
    if st.button("✨ Generate Question Paper PDF"):
        ...
        # Progress bar dikhane ke liye
        progress_bar = st.progress(0)
        status_text = st.empty()
        
        try:
            # Har ek photo ko AI se padhwa rahe hain
            ...
            st.error(f"Kuch error aayi. Kripya check karein ki aapki API key sahi hai ya nahi. Error: {e}")
